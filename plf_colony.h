...
#define PLF_COLONY_DEFINES  // ie. No encapsulating unit/class has previously defined the compiler feature macros in plf_tools.h below, so
                            // allow this header to undefine them at it's end.
#endif
...
inline constexpr auto plf_assert(bool expr) -> void {
    if (!expr) std::abort();
}
}  // namespace plf
...
struct limits {
    std::size_t   min, max;
    PLF_CONSTFUNC limits(const std::size_t minimum, const std::size_t maximum) PLF_NOEXCEPT : min(minimum), max(maximum) {}
};
...
class colony : private allocator_type  // Empty base class optimisation - inheriting allocator functions
{
    ...
    typedef typename plf::conditional<(priority == performance && (sizeof(element_type) > 10 || alignof(element_type) > 10)),
                                      unsigned short, unsigned char>::type
        skipfield_type;  // Note: unsigned short is equivalent to uint_least16_t ie. Using 16-bit unsigned integer in best-case scenario,
                         // greater-than-16-bit unsigned integer where platform doesn't support 16-bit types. unsigned char is always == 1
                         // byte, as opposed to uint_8, which may not be.
#else
    typedef typename plf::conditional<(priority == performance && sizeof(element_type) > 10), unsigned short, unsigned char>::type
        skipfield_type;
#endif
   ...
   public:
...
#ifdef PLF_ALIGNMENT_SUPPORT
    // The element as allocated in memory needs to be at-least 2*skipfield_type width in order to support free list indexes in erased
    // element memory space, so: make the size of this struct the larger of alignof(T), sizeof(T) or 2*skipfield_type (the latter is only
    // relevant for type char/uchar), and make the alignment alignof(T). This type is used mainly for correct pointer arithmetic while
    // iterating over elements in memory.
    struct alignas(alignof(element_type)) aligned_element_struct {
        // Using char as sizeof is always guaranteed to be 1 byte regardless of the number of bits in a byte on given computer, whereas for
        // example, uint8_t would fail on machines where there are more than 8 bits in a byte eg. Texas Instruments C54x DSPs.
        char data[(sizeof(element_type) < (sizeof(skipfield_type) * 2))
    ...
    // We combine the allocation of elements and skipfield into one allocation to save performance. This memory must be allocated as an
    // aligned type with the same alignment as T in order for the elements to align with memory boundaries correctly (which won't happen if
    // we allocate as char or uint_8). But the larger the sizeof in the type we use for allocation, the greater the chance of creating a lot
    // of unused memory in the skipfield portion of the allocated block. So we create a type that is sizeof(alignof(T)), as in most cases
    // alignof(T) < sizeof(T). If alignof(t) >= sizeof(t) this makes no difference.
   ...
   private:
    // Calculate the capacity of a group's elements+skipfield memory block when expressed in multiples of the value_type's alignment
    // (rounding up).
    ...
    typedef aligned_element_struct*
                            aligned_pointer_type;  // pointer to the (potentially overaligned) element type, not the original element type
    ...
    // (1) Follows low-complexity jump-counting pattern rules as described here:
    // archive.org/details/matt_bentley_-_the_low_complexity_jump-counting_pattern (2) Initialized to 0 by-default, which means 'non-erased'
    // ie. either no element has ever been constructed there, or an element has been constructed there. Whereas non-zero means an element
    // has been constructed there and subsequently erased. The value of the first and last non-zero nodes in a run of non-zero nodes
    // determines jump length. See the paper for details. (3) This definition means we can bulk-initialize each group's skipfield to 0,
    // rather than bulk-initialize to non-zero then subsequently change individual skipfield nodes to 0 upon insertion - which is obviously
    // slower. Defining unconstructed elements as 0 has no impact on iteration since they're after end(). Note that this definition is
    // violated slightly during splice: unconstructed element nodes at the end of the destination colony's back block will have their
    // corresponding skipfield nodes flipped to 'erased' in order to make iteration work, because they will not longer be after end() once
    // the source colony's blocks are appended. (4) There will always be one additional skipfield node allocated compared to the group's
    // number of elements. This ensures a faster ++ iterator operation (fewer checks are required when it is present). The extra node is
    // unused and always 0, but checked, and not having it will result in out-of-bounds memory errors.

    struct group {
        skipfield_pointer_type
            skipfield;  // Skipfield storage. The element and skipfield arrays are allocated contiguously, in a single allocation, in this
                        // implementation, hence the skipfield pointer also functions as a 'one-past-end' pointer for the elements array.
                        // This is present before elements in the group struct as it is referenced constantly by the ++ operator, hence
                        // having it first results in a minor performance increase.
        group_pointer_type next_group;  // Next group in the linked list of all groups. nullptr if no following group. 2nd in struct because
                                        // it is so frequently used during iteration.
        const aligned_struct_pointer_type
            elements;  // Element storage. Allocated as a block of chars, this memory is then divided between elements & skipfield
        group_pointer_type previous_group;  // Previous group in the linked list of all groups. nullptr if no preceding group.
        skipfield_type free_list_head;  // The index of the last erased element in the group. The last erased element will, in turn, contain
                                        // the number of the index of the next erased element, and so on. If this is == maximum
                                        // skipfield_type value then free_list is empty ie. no erasures have occurred in the group (or if
                                        // they have, the erased locations have subsequently been reused via insert/emplace/assign).
        const skipfield_type
            capacity;  // The element capacity of this particular group - can also be calculated from
                       // reinterpret_cast<aligned_pointer_type>(group->skipfield) - group->elements, however this space is effectively free
                       // due to struct padding and the sizeof(skipfield_type), and calculating it once is faster in benchmarking.
        skipfield_type size;  // The total number of active elements in group - changes with insert and erase commands - used to check for
                              // empty group in erase function, as an indication to remove the group. Also used in combination with capacity
                              // to check if group is full, which is used in the next/previous/advance/distance overloads, and range-erase.
        group_pointer_type erasures_list_next_group,
            erasures_list_previous_group;  // The next and previous groups in the list of groups with erasures ie. with active
                                           // erased-element free lists. nullptr if no next or previous group.
        size_type group_number;            // Used for comparison (> < >= <= <=>) iterator operators (used by distance function and user).

#ifdef PLF_VARIADICS_SUPPORT
        group(aligned_struct_allocator_type& aligned_struct_allocator, const skipfield_type elements_per_group,
              const group_pointer_type previous)
            : next_group(nullptr),
              elements(PLF_ALLOCATE(aligned_struct_allocator_type, aligned_struct_allocator, get_aligned_block_capacity(elements_per_group),
                                    (previous == nullptr) ? nullptr : previous->elements)),
...
#else
                                 // This is a hack around the fact that allocator_type::construct only supports copy construction in C++03
                                 // and copy elision does not occur on the vast majority of compilers in this circumstance. So to avoid
                                 // running out of memory (and losing performance) from allocating the same block twice, we 'move' in the
                                 // 'copy' constructor.
        group(aligned_struct_allocator_type& aligned_struct_allocator, const skipfield_type elements_per_group,
              const group_pointer_type previous) PLF_NOEXCEPT
            : elements(PLF_ALLOCATE(aligned_struct_allocator_type, aligned_struct_allocator, get_aligned_block_capacity(elements_per_group),
        ...
        group(const group& source)
            : skipfield(pointer_cast<skipfield_pointer_type>(source.front() + source.capacity)),
              ...
              erasures_list_next_group(nullptr),
              erasures_list_previous_group(nullptr),
              group_number((source.previous_group == nullptr) ? 0 : source.previous_group->group_number + 1u) {
            std::memset(plf::void_cast(skipfield), 0, sizeof(skipfield_type) * (static_cast<size_type>(capacity) + 1u));
        }
#endif

        void reset(const skipfield_type increment, const group_pointer_type next, const group_pointer_type previous,
                   const size_type group_num) PLF_NOEXCEPT {
            next_group                   = next;
            free_list_head               = std::numeric_limits<skipfield_type>::max();
            previous_group               = previous;
            size                         = increment;
            ...
            std::memset(
                plf::void_cast(skipfield), 0,
                sizeof(skipfield_type) * static_cast<size_type>(capacity));  // capacity + 1 is not necessary here as the final skipfield
                                                                             // node is never written to after initialization
        }

        bool no_erasures() const PLF_NOEXCEPT { return free_list_head == std::numeric_limits<skipfield_type>::max(); }

        aligned_pointer_type first_element() const PLF_NOEXCEPT { return pointer_cast<aligned_pointer_type>(elements) + *skipfield; }

        aligned_pointer_type front() const PLF_NOEXCEPT { return pointer_cast<aligned_pointer_type>(elements); }

        aligned_pointer_type past_back() const PLF_NOEXCEPT { return pointer_cast<aligned_pointer_type>(skipfield); }
    };
    ...
    group_pointer_type
        erasure_groups_head,  // Head of doubly-linked list of groups which have erased-element memory locations available for re-use
    ...
    // Under most compilers, the following will be 1 byte each - since the skipfield types are 2 bytes each at max, the following add
    // nothing to the class sizeof in a 64-bit build, due to padding
    ...
    void blank() PLF_NOEXCEPT {
        ...
        if PLF_CONSTEXPR (std::is_standard_layout<colony>::value && std::allocator_traits<allocator_type>::is_always_equal::value &&
                          std::is_trivially_destructible<group_pointer_type>::value) {  // If all pointer types are trivial, we can just
                                                                                        // nuke the member variables from orbit with memset
                                                                                        // (nullptr is always 0):
            std::memset(static_cast<void*>(this), 0, reinterpret_cast<char*>(&min_block_capacity) - reinterpret_cast<char*>(this));
        } else
#endif
        {
    ...
    static PLF_CONSTFUNC std::size_t max_size_static() PLF_NOEXCEPT {
#ifdef PLF_ALLOCATOR_TRAITS_SUPPORT
        return static_cast<std::size_t>(std::allocator_traits<allocator_type>::max_size(allocator_type()));
#else
        return static_cast<std::size_t>(std::numeric_limits<size_type>::max() /
                                        sizeof(element_type));  // Substituting because allocator::max_size is not static
#endif
    }
   ...
   public:
    ...
    static PLF_CONSTFUNC skipfield_type block_capacity_default_min() PLF_NOEXCEPT {
        const skipfield_type adaptive_size =
            static_cast<skipfield_type>(((sizeof(colony) + sizeof(group)) * 2) / sizeof(aligned_element_struct));
        const skipfield_type max_block_capacity =
            block_capacity_default_max();  // Necessary to check against in situations with > 64bit pointer sizes and small sizeof(T)
        return std::max(static_cast<skipfield_type>(8), std::min(adaptive_size, max_block_capacity));
    }

    // Adaptive maximum based on numeric_limits and best outcome from multiple benchmark's (on balance) in terms of memory usage and
    // performance:
    static PLF_CONSTFUNC skipfield_type block_capacity_default_max() PLF_NOEXCEPT {
        return static_cast<skipfield_type>(
            std::min(std::min(static_cast<std::size_t>(std::numeric_limits<skipfield_type>::max()), static_cast<std::size_t>(8192u)),
                     max_size_static()));
    }

    static PLF_CONSTFUNC plf::limits block_capacity_default_limits() PLF_NOEXCEPT {
        return plf::limits(static_cast<std::size_t>(block_capacity_default_min()), static_cast<std::size_t>(block_capacity_default_max()));
    }

    // Default constructors:

    PLF_CONSTFUNC explicit colony(const allocator_type& alloc) PLF_NOEXCEPT : allocator_type(alloc),
                                                                              erasure_groups_head(nullptr),
                                                                              unused_groups_head(nullptr),
                                                                              total_size(0),
                                                                              total_capacity(0),
    ...
    PLF_CONSTFUNC colony() PLF_NOEXCEPT_ALLOCATOR : erasure_groups_head(nullptr),
                                                    unused_groups_head(nullptr),
                                                    total_size(0),
                                                    total_capacity(0),
                                                    min_block_capacity(block_capacity_default_min()),
...
// Copy constructors:
    ...
    colony(const colony& source, const allocator_type& alloc)
        :
          ...
          total_capacity(0),
          min_block_capacity(
              std::max(source.min_block_capacity,
                       static_cast<skipfield_type>(std::min(
                           source.total_size,
                           static_cast<size_type>(
                               source.max_block_capacity))))),  // min group size is set to value closest to total number of elements in
                                                                // source colony, in order to not create unnecessary small groups in the
                                                                // range-insert below, then reverts to the original min group size
                                                                // afterwards. This effectively saves a call to reserve.
          max_block_capacity(source.max_block_capacity),
          group_allocator(*this),
          aligned_struct_allocator(*this),
          skipfield_allocator(*this),
          tuple_allocator(*this) {  // can skip checking for skipfield conformance here as source will have already checked theirs. Same
                                    // applies for other copy and move constructors below
    ...
    colony(const colony& source)
        :
          ...
          min_block_capacity(
              std::max(source.min_block_capacity,
                       static_cast<skipfield_type>(std::min(source.total_size, static_cast<size_type>(source.max_block_capacity))))),
          max_block_capacity(source.max_block_capacity),
          group_allocator(*this),
          aligned_struct_allocator(*this),
          skipfield_allocator(*this),
    ...
    colony(colony&& source) PLF_NOEXCEPT : allocator_type(static_cast<allocator_type&>(source)),
    ...
    colony(const size_type fill_number, const element_type& element, const plf::limits block_limits,
    ...
    colony(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type& first,
           const iterator_type& last, const plf::limits block_limits, const allocator_type& alloc = allocator_type())
        : allocator_type(alloc),
          erasure_groups_head(nullptr),
          unused_groups_head(nullptr),
          total_size(0),
    ...
    template <typename iterator_type>
    colony(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type& first,
           const iterator_type& last, const allocator_type& alloc = allocator_type())
        : allocator_type(alloc),
          erasure_groups_head(nullptr),
          unused_groups_head(nullptr),
          total_size(0),
    ...
    colony(const std::initializer_list<element_type>& element_list, const plf::limits block_limits,
           const allocator_type& alloc = allocator_type())
        : allocator_type(alloc),
          erasure_groups_head(nullptr),
          unused_groups_head(nullptr),
    ...
    iterator begin() PLF_NOEXCEPT { return begin_iterator; }

    const_iterator begin() const PLF_NOEXCEPT { return begin_iterator; }

    iterator end() PLF_NOEXCEPT { return end_iterator; }

    const_iterator end() const PLF_NOEXCEPT { return end_iterator; }

    const_iterator cbegin() const PLF_NOEXCEPT { return begin_iterator; }

    const_iterator cend() const PLF_NOEXCEPT { return end_iterator; }

    reverse_iterator rbegin() PLF_NOEXCEPT { return reverse_iterator(end_iterator); }

    const_reverse_iterator rbegin() const PLF_NOEXCEPT { return crbegin(); }

    reverse_iterator rend() PLF_NOEXCEPT { return reverse_iterator(begin_iterator); }

    const_reverse_iterator rend() const PLF_NOEXCEPT { return crend(); }

    const_reverse_iterator crbegin() const PLF_NOEXCEPT { return const_reverse_iterator(end_iterator); }

    const_reverse_iterator crend() const PLF_NOEXCEPT { return const_reverse_iterator(begin_iterator); }

    ~colony() PLF_NOEXCEPT { destroy_all_data(); }

   private:
    group_pointer_type allocate_new_group(const skipfield_type elements_per_group, const group_pointer_type previous = nullptr) {
        if (max_size() - total_capacity < elements_per_group)  // Just in case max_size is a lower amount than the actual memory available
                                                               // (uncommon platform). Comparison avoids overflow.
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
        ...
        total_capacity += elements_per_group;  // I don't know why GCC creates better/smaller codegen when this is placed here rather than
    ...
    void deallocate_group(const group_pointer_type the_group) PLF_NOEXCEPT {
        PLF_DEALLOCATE(aligned_struct_allocator_type, aligned_struct_allocator, the_group->elements,
    ...
    defined(PLF_CPP20_SUPPORT)  // To avoid codegen for this function with trivially-destructible types. CPP20 because we don't want to
                                // trigger a branch for every destruction
    ...
    void destroy_all_data() PLF_NOEXCEPT {
        if (begin_iterator.group_pointer != nullptr) {
            ...
            {
                if (total_size != 0) {
                    while (begin_iterator.group_pointer !=
                           end_iterator.group_pointer)  // Erase elements without bothering to update skipfield - much faster:
                    {
    ...
    void edit_free_list_prev(const aligned_pointer_type location, const skipfield_type value)
        PLF_NOEXCEPT  // Write to the 'previous erased element' index in the erased element memory location
    {
    ...
    void update_skipblock(const iterator& new_location, const skipfield_type prev_free_list_index) PLF_NOEXCEPT {
        ...
        } else  // single-node skipblock, remove skipblock
        {
            ...
            {
                erasure_groups_head =
                    erasure_groups_head->erasures_list_next_group;  // update_skipblock is only used within insert/emplace, where the head
                                                                    // group is being used, so no need for additional checks here
            }
        }
        ...
        *(new_location.skipfield_pointer) = 0;
        ...
        if (new_location.group_pointer == begin_iterator.group_pointer &&
            new_location.element_pointer <
                begin_iterator
                    .element_pointer) { /* ie. begin_iterator was moved forwards as the result of an erasure at some point, this erased
                                           element is before the current begin, hence, set current begin iterator to this element */
    ...
    void reset_group_numbers() PLF_NOEXCEPT { update_subsequent_group_numbers(0, begin_iterator.group_pointer); }
   ...
   public:
    ...
    iterator insert(const element_type& element) {
        if (end_iterator.element_pointer != nullptr) {
            ...
            {
                const iterator new_location(erasure_groups_head, erasure_groups_head->front() + erasure_groups_head->free_list_head,
                                            erasure_groups_head->skipfield + erasure_groups_head->free_list_head);

                // We always reuse the element at the start of the skipblock, this is also where the free-list information for that
                // skipblock is stored. Get the previous free-list node's index from this memory space, before we write to our element to
                // it. 'Next' index is always the free_list_head (as represented by the maximum value of the skipfield type) here so we
...
#ifdef PLF_CPP20_SUPPORT
    iterator insert([[maybe_unused]] const_iterator& hint,
                    const element_type&              element)  // Note: hint is ignored, purely to serve other standard library functions
    {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    iterator insert(element_type&& element)  // The move-insert function is near-identical to the regular insert function, with the
...
#ifdef PLF_CPP20_SUPPORT
    iterator insert([[maybe_unused]] const_iterator& hint, element_type&& element) { return insert(std::forward<element_type&&>(element)); }
#endif
#endif

#ifdef PLF_VARIADICS_SUPPORT
    template <typename... arguments>
    iterator emplace(arguments&&... parameters)  // The emplace function is near-identical to the regular insert function, with the
                                                 // exception of the element construction method, and change to is_nothrow tests.
    {
        if (end_iterator.element_pointer != nullptr) {
            if (erasure_groups_head == nullptr) {
                if (end_iterator.element_pointer != end_iterator.group_pointer->past_back()) {
                ...
                if (unused_groups_head == nullptr) {
                    next_group =
                        allocate_new_group(static_cast<skipfield_type>(std::min(total_size, static_cast<size_type>(max_block_capacity))),
                                           end_iterator.group_pointer);
...
#ifdef PLF_EXCEPTIONS_SUPPORT
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR ((!std::is_copy_constructible<element_type>::value && !std::is_nothrow_move_constructible<element_type>::value) ||
                          !std::is_nothrow_copy_constructible<element_type>::value)  // to avoid unnecessary codegen, since this function
                                                                                     // will never be called if this line isn't true
#endif
        {
            const skipfield_type elements_constructed_before_exception =
                static_cast<skipfield_type>(end_iterator.element_pointer - end_iterator.group_pointer->front());
    ...
    void fill(const element_type& element, const skipfield_type size) {
        ...
        if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
            if PLF_CONSTEXPR (sizeof(aligned_element_struct) != sizeof(element_type)) {
                alignas(alignof(aligned_element_struct)) element_type aligned_copy =
                    element;  // to avoid potentially violating memory boundaries in line below, create an initial object copy of same (but
                              // aligned) type
                plf::uninitialized_fill_n(end_iterator.element_pointer, size, *pointer_cast<aligned_pointer_type>(&aligned_copy),
                                          static_cast<allocator_type&>(*this));
            } else {
                plf::uninitialized_fill_n(pointer_cast<pointer>(end_iterator.element_pointer), size, element,
                                          static_cast<allocator_type&>(*this));
            }
...
// For catch blocks in range_fill_skipblock and fill_skipblock - update existing skipblock and free-list indexes to reflect partially-reused
// skipblock:
#ifdef PLF_CPP20_SUPPORT
    void recover_from_partial_skipblock_fill([[maybe_unused]] const aligned_pointer_type   location,
                                             [[maybe_unused]] const aligned_pointer_type   current_location,
                                             [[maybe_unused]] const skipfield_pointer_type skipfield_pointer,
                                             [[maybe_unused]] const skipfield_type         prev_free_list_node)
#else
    void recover_from_partial_skipblock_fill(const aligned_pointer_type location, const aligned_pointer_type current_location,
                                             const skipfield_pointer_type skipfield_pointer, const skipfield_type prev_free_list_node)
#endif
    {
...
#endif
        {
            ...
            std::memset(
                plf::void_cast(skipfield_pointer), 0,
                elements_constructed_before_exception * sizeof(skipfield_type));  // Reset skipfield for elements written before exception
            *new_start_node = *(new_start_node + new_start_node_value - 1) =
                new_start_node_value;  // Create new skipblock for unused elements
    ...
    void fill_skipblock(const element_type& element, const aligned_pointer_type location, const skipfield_pointer_type skipfield_pointer,
                        const skipfield_type size) {
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
            if PLF_CONSTEXPR (sizeof(aligned_element_struct) != sizeof(element_type)) {
                alignas(alignof(aligned_element_struct)) element_type aligned_copy = element;
                plf::uninitialized_fill_n(location, size, *pointer_cast<aligned_pointer_type>(&aligned_copy),
                                          static_cast<allocator_type&>(*this));
            } else {
...
#endif
        {
            ...
            const skipfield_type prev_free_list_node =
                *pointer_cast<skipfield_pointer_type>(location);  // in case of exception, grabbing indexes before free_list node is reused
#endif
    ...
    void fill_unused_groups(size_type size, const element_type& element, size_type group_number, group_pointer_type previous_group,
   ...
   public:
    ...
    void insert(size_type size, const element_type& element) {
        ...
        while (erasure_groups_head !=
               nullptr)  // skipblock loop: breaks when colony is exhausted of reusable skipblocks, or returns if size == 0
        {
            ...
            if (skipblock_size <= size) {
                erasure_groups_head->free_list_head =
                    *pointer_cast<skipfield_pointer_type>(element_pointer);  // set free list head to previous free list node
                ...
                {
                    edit_free_list_next(
                        erasure_groups_head->front() + erasure_groups_head->free_list_head,
            ...
            } else  // skipblock is larger than remaining number of elements
            {
                const skipfield_type prev_index =
                    *pointer_cast<skipfield_pointer_type>(element_pointer);  // save before element location is overwritten
                ...
                *(skipfield_pointer + skipblock_size - 1) = new_skipblock_size;
                ...
                if (prev_index != std::numeric_limits<skipfield_type>::max()) {
                    edit_free_list_next(
                        erasure_groups_head->front() + prev_index,
                        erasure_groups_head->free_list_head);  // set 'next' index of previous skipblock to new start of skipblock
                }
        ...
        const skipfield_type group_remainder = static_cast<skipfield_type>(
            std::min(static_cast<size_type>(end_iterator.group_pointer->past_back() - end_iterator.element_pointer), size));
   ...
   private:
...
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
            do { PLF_CONSTRUCT_ELEMENT(end_iterator.element_pointer, *it++); } while (++end_iterator.element_pointer != fill_end);
        }
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        else if PLF_CONSTEXPR (std::is_nothrow_move_constructible<element_type>::value &&
    ...
    template <class iterator_type>
    void range_fill_skipblock(iterator_type& it, const aligned_pointer_type location, const skipfield_pointer_type skipfield_pointer,
                              const skipfield_type size) {
        const aligned_pointer_type fill_end = location + size;
    ...
    template <class iterator_type>
    void range_fill_unused_groups(size_type size, iterator_type it, size_type group_number, group_pointer_type previous_group,
                                  const group_pointer_type current_group) {
        for (end_iterator.group_pointer = current_group; end_iterator.group_pointer->capacity < size;
             end_iterator.group_pointer = end_iterator.group_pointer->next_group) {
            const skipfield_type capacity = end_iterator.group_pointer->capacity;
    ...
    template <class iterator_type>
    void range_insert(iterator_type it,
                      size_type size)  // this is near-identical to the fill insert, with the only alteration being incrementing an iterator
                                       // for construction, rather than using a const element. And the fill etc function calls are changed
                                       // to range_fill to match this pattern. See fill insert for code explanations
    {
        if (size == 0) {
            return;
        } else if (size == 1) {
        ...
        while (erasure_groups_head != nullptr) {
            ...
            if (skipblock_size <= size) {
                ...
                if (!erasure_groups_head->no_erasures()) {
                    edit_free_list_next(erasure_groups_head->front() + erasure_groups_head->free_list_head,
   ...
   public:
    ...
    template <class iterator_type>
    void insert(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first,
   ...
   private:
    void add_to_groups_with_erasures_list(const group_pointer_type group_to_add) PLF_NOEXCEPT {
        ...
        if (erasure_groups_head != nullptr) { erasure_groups_head->erasures_list_previous_group = group_to_add; }
   ...
   public:
    ...
    {
        plf_assert(total_size != 0);
        plf_assert(it.group_pointer != nullptr);                         // ie. not uninitialized iterator
        plf_assert(it.element_pointer != end_iterator.element_pointer);  // ie. != end()
        plf_assert(*(it.skipfield_pointer) == 0);  // ie. element pointed to by iterator has not been erased previously
        ...
        if (--(it.group_pointer->size) != 0)  // ie. non-empty group at this point in time, don't consolidate
        {
            ...
            // If node has skipblocks on left and right, set start node of left skipblock and end node of right skipblock to the values of
            // the left + right nodes + 1

            // Optimization explanation:
            // The contextual logic below is the same as that in the insert() functions but in this case the value of the current skipfield
            // node will always be zero (since it is not yet erased), meaning no additional manipulations are necessary for the previous
            // skipfield node comparison - we only have to check against zero
            const skipfield_type prev_skipfield =
                *(it.skipfield_pointer -
                  (it.skipfield_pointer !=
                   it.group_pointer->skipfield));  // true if previous node is erased or this node is at beginning of skipfield
            const skipfield_type after_skipfield =
                *(it.skipfield_pointer +
                  1);  // NOTE: boundary test (checking against end-of-elements) is able to be skipped due to the extra skipfield node
                       // (compared to element field) - which is present to enable faster iterator operator ++ operations
            ...
            if ((prev_skipfield == 0) & (after_skipfield == 0))  // no consecutive erased elements
            {
                *it.skipfield_pointer      = 1;  // solo skipped node
                ...
                if (!it.group_pointer->no_erasures()) {
                    edit_free_list_next(it.group_pointer->front() + it.group_pointer->free_list_head,
            ...
            } else if ((prev_skipfield == 0) & (after_skipfield != 0))  // following erased consecutive elements, none preceding
            {
                *(it.skipfield_pointer + after_skipfield) = *(it.skipfield_pointer) = after_skipfield + 1;
                ...
                if (following_previous != std::numeric_limits<skipfield_type>::max()) {
                    edit_free_list_next(it.group_pointer->front() + following_previous,
                ...
                if (following_next != std::numeric_limits<skipfield_type>::max()) {
                    edit_free_list_prev(it.group_pointer->front() + following_next,
            ...
            {
                *(it.skipfield_pointer) = 1;  // This line necessary in order for get_iterator() to work - ensures that erased element
                                              // skipfield nodes are always non-zero

                // Join the skipblocks
                *(it.skipfield_pointer - prev_skipfield) = *(it.skipfield_pointer + after_skipfield) =
                    static_cast<skipfield_type>(prev_skipfield + after_skipfield + 1);
            ...
            iterator return_iterator(it.group_pointer, it.element_pointer + after_skipfield + 1,
            ...
            if (it.element_pointer == begin_iterator.element_pointer)
                begin_iterator =
                    return_iterator;  // If original iterator was first element in hive, update it's value with the next non-erased element:

            return return_iterator;
        }

        // else: group is empty, consolidate groups
        const bool in_back_block  = (it.group_pointer == end_iterator.group_pointer),
                   in_front_block = (it.group_pointer == begin_iterator.group_pointer);
        ...
        } else if ((!in_back_block) & in_front_block)  // ie. Remove first group, change first group to next group
        {
            ...
            return begin_iterator;
        } else if (!(in_back_block | in_front_block))  // this is a non-first group but not final group in chain: delete the group, then
                                                       // link previous group to the next group in the chain:
        {
            it.group_pointer->next_group->previous_group = it.group_pointer->previous_group;
            const group_pointer_type return_group        = it.group_pointer->previous_group->next_group =
                it.group_pointer->next_group;  // close the chain, removing this group from it
            remove_from_groups_with_erasures_list(
                it.group_pointer);  // As above, not a back group, hence will always have prior erasures at this point.

            if PLF_CONSTEXPR (priority == performance) {
                if (it.group_pointer->next_group != end_iterator.group_pointer) {
                    deallocate_group_remove_capacity(it.group_pointer);
                } else {  // ie. second to last block in iterative sequence
            ...
            return iterator(return_group, return_group->front() + *(return_group->skipfield),
                            return_group->skipfield + *(return_group->skipfield));
        } else  // this is a non-first group and the final group in the chain
        {
            remove_from_groups_with_erasures_list_if_necessary(
                it.group_pointer);  // Will not have prior erasures if only one element was ever inserted into this group and that was the
                                    // element we erased.
            it.group_pointer->previous_group->next_group = nullptr;
            end_iterator.group_pointer =
                it.group_pointer
                    ->previous_group;  // end iterator needs to be changed as element supplied was the back element of the colony
            ...
            if PLF_CONSTEXPR (priority == performance) {
                ...
                if (unused_groups_head !=
                    nullptr)  // priority == memory_use, if there are other reserved blocks already, get rid of this one
                {
                    deallocate_group_remove_capacity(it.group_pointer);
   ...
   private:
    void partially_erase_group(const const_iterator& start, const aligned_pointer_type end) {
        // For the partial block erasures, we have to remove the existing skipblocks within the range from the intra-block free list of
        // skipblocks. However if there're no erasures in the block, we can avoid doing so.
        const_iterator current       = start;
        skipfield_type erasure_count = 0;

        // First erase all elements until end of block & remove all skipblocks post-initial position from the free_list. Then, either update
        // preceding skipblock or create new one:
                ...
                if (*current.skipfield_pointer != 0) {
                            ...
                            std::numeric_limits<skipfield_type>::max())  // if this is the last skipblock in the free list
                    {
...
#endif
                        {
                            while (current.element_pointer != end)
                                destroy_element(
                                    current.element_pointer++);  // Avoids checking skipfield, as there are no erased elements left in block
                        }
                    ...
                    } else if (next_free_list_index == std::numeric_limits<skipfield_type>::max())  // if this is the head of the free list
                    {
                        ...
                        edit_free_list_next(current.group_pointer->front() + prev_free_list_index,
                                            std::numeric_limits<skipfield_type>::max());
        ...
        if (previous_node_value ==
            0)  // start element is either at start of block, or previous element is non-erased so no adjacent skipblock
        {
            *(start.skipfield_pointer) = *(start.skipfield_pointer + distance_to_end - 1) =
                static_cast<skipfield_type>(distance_to_end);  // set start and end node of skipblock
            ...
            *(start.skipfield_pointer - previous_node_value) = *(start.skipfield_pointer + distance_to_end - 1) =
                static_cast<skipfield_type>(previous_node_value + distance_to_end);
        }

        if (distance_to_end >
            2)  // if the skipblock is longer than 2 nodes, fill in the middle nodes with non-zero values so that get_iterator() will work
        {
   ...
   public:
    ...
    iterator erase(const const_iterator& iterator1,
                   const const_iterator& iterator2)  // if uninitialized/invalid iterators supplied, function could generate an exception.
                                                     // If iterator1 > iterator2, behaviour is undefined.
    {
        // General code logic: if iterator1 and iterator2 point to elements in the same block, we skip to code section 3 (final block).
        // If they aren't and iterator1 isn't the first non-erased element in first block, we erase part of that block and update
        // accordingly in code Section 1. If it is the first non-erased element, it gets handled in code section 2. In code Section 2 we
        // fully erase and remove all intermediate blocks which aren't the final block. This's optimal as we can just discard the blocks and
        // not do any skipfield or free list updating. In code Section 3 we either partially or fully erase (if iterator2 == end()) the
        // final block in the supplied sequence. If iterator2 was the first non-erased element in it's block or iterator1 == iterator2, this
        // is caught at this point and no action is taken.

        plf_assert(iterator1 <= iterator2);
        ...
        if (iterator1.group_pointer != iterator2.group_pointer) {
            ...
            if (current.element_pointer !=
                current.group_pointer
                    ->first_element())  // if iterator1 is not the first non-erased element in it's block - most common case
            {
                partially_erase_group(iterator1, iterator1.group_pointer->past_back());
                current.group_pointer = current.group_pointer->next_group;
            }

            // Section 2: remove all intermediate blocks before final block (including first block if it's a full block erasure rather than
            // partial)
            // ====================================================================================================================================
            ...
            if (previous_group != nullptr) {
                ...
                begin_iterator = iterator(iterator2.group_pointer, iterator2.element_pointer,
                                          iterator2.skipfield_pointer);  // This line is included primarily to avoid a secondary if
                                                                         // statement within the if block below - it is not needed otherwise
            }
        }
        ...
        // =======================================================================
        // Code logic:
        // If not erasing entire final block, 1. Destruct elements (if non-trivial destructor), 2. add skipblock location to group's free
        // list of skipblocks, and 3. update skipfield. If erasing entire block, 1. Destruct elements (if non-trivial destructor), 2. if no
        // elements left in hive reset the group, otherwise 3. reset end_iterator and remove group from groups-with-erasures list (if prior
        // erasures are present). Note: only way that entire block can be erased is if iterator2 == end() in this case, hence why we reset
        // end_iterator.

        if (current.element_pointer !=
            iterator2.element_pointer)  // in case iterator2 was at beginning of it's block - also covers empty range case (first == last)
        {
            if (iterator2.element_pointer != end_iterator.element_pointer ||
                current.element_pointer !=
                    current.group_pointer->first_element())  // ie. not erasing entire block. Second condition can only (potentially) be
                                                             // true if iterator1 & iterator2 are in same block.
            {
                partially_erase_group(current, iterator2.element_pointer);
                // Note: there is no need to check for an immediately-adjacent skipblock following the range, as that would imply that
                // iterator2 pointed to an erased element, making it an invalid iterator and unable to be supplied to erase().
            ...
            } else  // ie. full block erasure - this can only happen if iterator2 = end() - as it's not valid for an iterator to be pointing
                    // one-past the end of a block unless that is the current end().
            {
#ifdef PLF_TYPE_TRAITS_SUPPORT
                if PLF_CONSTEXPR (!std::is_trivially_destructible<element_type>::value)
#endif
...
// Destroy all elements if non-trivial:
    ...
    void assign(size_type size, const element_type& element) {
...
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR ((std::is_trivially_destructible<element_type>::value && std::is_trivially_constructible<element_type>::value &&
                           std::is_trivially_copy_assignable<element_type>::value) ||
                          !std::is_copy_assignable<element_type>::value)  // ie. If there is no benefit nor difference to assigning vs
                                                                          // constructing, or if we can't assign, use faster method:
        {
            prepare_groups_for_assign(size);
            fill_unused_groups(size, element, 0, nullptr, begin_iterator.group_pointer);
        } else
#endif
        {
            if (total_size == 0) {
                ...
                do { *current++ = element; } while (--size != 0);

                erase(current, end_iterator);
            } else {
                iterator current = begin_iterator;

                do { *current = element; } while (++current != end_iterator);
    ...
    void range_assign(iterator_type it, size_type size) {
        ...
        if (total_size == 0) {
            ...
            for (iterator current(begin_iterator); current.group_pointer != nullptr;
                ...
                for (const aligned_pointer_type end = (current.group_pointer == end_iterator.group_pointer)
                                                          ? end_iterator.element_pointer
                                                          : current.group_pointer->past_back();
                     current.element_pointer != end;) {
                    if (*(current.skipfield_pointer) != 0) {
                        skipfield_type skipblock_length = *(current.skipfield_pointer);
                        const_iterator next_element(current.group_pointer, current.element_pointer + skipblock_length,
                        ...
                        for (const aligned_pointer_type fill_end = current.element_pointer + skipblock_length;
                             current.element_pointer != fill_end; ++current.element_pointer) {
#ifdef PLF_EXCEPTIONS_SUPPORT
#ifdef PLF_TYPE_TRAITS_SUPPORT
                            if PLF_CONSTEXPR (std::is_nothrow_copy_constructible<element_type>::value) {
...
#endif
                            {
                                try {
                                    ...
                                    current.skipfield_pointer =
                                        current.group_pointer->skipfield + (current.element_pointer - current.group_pointer->front());
            ...
            // Use up any remaining space at end of end block (would not be correctly identified above because the skipfield in unused nodes
            // is 0)
   ...
   public:
    ...
    template <class iterator_type>
    void assign(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type& first,
                const iterator_type&                                                                                 last) {
        range_assign(first, static_cast<size_type>(std::distance(first, last)));
    }

    // Overloads for colony iterators, since std::distance overloads for these are not possible without C++20 concepts and we must stick
    // with ADL:
    ...
    size_type size() const PLF_NOEXCEPT { return total_size; }
    ...
    size_type capacity() const PLF_NOEXCEPT { return total_capacity; }

    size_type memory() const PLF_NOEXCEPT {
        size_type memory_use                   = sizeof(*this);       // sizeof colony structure
        end_iterator.group_pointer->next_group = unused_groups_head;  // temporarily link the active & reserved (unused) groups in order to
                                                                      // only have one loop below instead of two
        ...
        for (group_pointer_type current = begin_iterator.group_pointer; current != nullptr; current = current->next_group) {
            memory_use +=
                sizeof(group) + (get_aligned_block_capacity(current->capacity) *
                                 sizeof(aligned_allocation_struct));  // add element/skipfield memory block sizes + size of the group struct
        }
    ...
    static PLF_CONSTFUNC size_type block_metadata_memory(const size_type block_capacity) PLF_NOEXCEPT {
        return sizeof(group) + ((get_aligned_block_capacity(static_cast<skipfield_type>(block_capacity)) - block_capacity) *
                                sizeof(aligned_allocation_struct));
    }
    ...
    static PLF_CONSTFUNC size_type max_elements_per_allocation(const size_type allocation_amount) PLF_NOEXCEPT {
        ...
        if ((/* Explanation: elements and skipfield are allocated in a single allocation to save performance.
                     ...
                     when allocated in multiples of the element type's alignof. The + sizeof(skipfield_type) adds the additional skipfield
                node as mentioned, and the (num_units + 1) minus 1 byte rounds up the integer division: */
            ...
            > allocation_amount) {
            --num_units;  // In this implementation it is not possible for the necessary adjustment to be greater than 1 element+skipfield
                          // sizeof
        }
   ...
   private:
    // get all elements contiguous in memory and shrink to fit, remove erasures and free lists. Invalidates all iterators and pointers to
    // elements.
    void consolidate(const skipfield_type new_min, const skipfield_type new_max) {
        ...
        if PLF_CONSTEXPR (!std::is_trivially_copyable<element_type>::value && std::is_nothrow_move_constructible<element_type>::value) {
            temp.range_fill_unused_groups(total_size, plf::make_move_iterator(begin_iterator), 0, nullptr,
   ...
   public:
    void reshape(const plf::limits block_limits) {
        check_capacities_conformance(block_limits);
        const skipfield_type new_min = static_cast<skipfield_type>(block_limits.min),
                             new_max = static_cast<skipfield_type>(block_limits.max);

        if (total_capacity != 0) {
            if (total_size != 0) {
                if (min_block_capacity > new_max ||
                    max_block_capacity < new_min)  // If none of the original blocks could potentially fit within the new limits, skip
                ...
                if (min_block_capacity < new_min ||
                    max_block_capacity > new_max)  // ie. If existing blocks could be outside of the new limits
                {
                    // Otherwise need to check all group sizes here (not just back one, which is most likely largest), because splice might
                    // append smaller blocks after a larger block:
            ...
            // If a consolidation or throw has not occured, process reserved/unused groups and deallocate where they don't fit the new
            // limits:
    ...
    PLF_CONSTFUNC plf::limits block_capacity_limits() const PLF_NOEXCEPT {
        return plf::limits(static_cast<std::size_t>(min_block_capacity), static_cast<std::size_t>(max_block_capacity));
    }

    static PLF_CONSTFUNC plf::limits block_capacity_hard_limits() PLF_NOEXCEPT {
        return plf::limits(3, std::min(static_cast<std::size_t>(std::numeric_limits<skipfield_type>::max()), max_size_static()));
    }
...
// Destroy all elements if element type is non-trivial:
        ...
        if (begin_iterator.group_pointer != end_iterator.group_pointer) {  // Move all other groups onto the unused_groups list
            end_iterator.group_pointer->next_group = unused_groups_head;
            unused_groups_head                     = begin_iterator.group_pointer->next_group;
...
#endif
            {
                if (static_cast<allocator_type&>(*this) !=
                    static_cast<const allocator_type&>(
                        source)) {  // Deallocate existing blocks as source allocator is not necessarily able to do so
                    reset();
                }
            }
...
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
        if PLF_CONSTEXPR ((std::is_trivially_copyable<allocator_type>::value ||
...
// Move assignment
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
    colony& operator=(colony&& source) PLF_NOEXCEPT(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value ||
                                                    std::allocator_traits<allocator_type>::is_always_equal::value)
#else
...
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
        if PLF_CONSTEXPR (std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value ||
                          std::allocator_traits<allocator_type>::is_always_equal::value) {  // Note: we need this to be constexpr to avoid
                                                                                            // warning errors on the potentially-throwing
                                                                                            // section below
    ...
    friend bool operator==(const colony& lh, const colony& rh) PLF_NOEXCEPT {
        ...
        for (const_iterator lh_iterator = lh.begin_iterator, rh_iterator = rh.begin_iterator; lh_iterator != lh.end_iterator;
             ++lh_iterator, ++rh_iterator) {
    ...
    friend bool operator!=(const colony& lh, const colony& rh) PLF_NOEXCEPT { return !(lh == rh); }
    ...
    void trim_capacity() PLF_NOEXCEPT {
        if (end_iterator.element_pointer == nullptr) return;  // empty colony
    ...
    void trim_capacity(const size_type capacity_retain) PLF_NOEXCEPT {
        ...
        if (begin_iterator.element_pointer == end_iterator.element_pointer)  // ie. clear() has been called prior
        {
            if (capacity_to_remove_remaining >= begin_iterator.group_pointer->capacity) {
                ...
                if (unused_groups_head != nullptr)  // some of the reserved blocks were not removed as they were too large, so use one of
                                                    // these to make the new begin group
                {
    ...
    void reserve(size_type new_capacity) {
        ...
        if (remainder == 0) {
            ...
            // This also means that if remainder < min_block_capacity we don't have to allocate a min capacity group and spread the
            // difference over subsequent groups (see subsequent if block). The smaller group is not deallocated immediately so that, in the
            // event that an exception is triggered when allocating the larger group, we don't end up with lower capacity than before
            // reserve().
            ...
            if (unused_groups_head != nullptr && max_block_capacity - remainder >= min_block_capacity) {
                ...
                do {
                    ...
                    // If there exists an unused group which's of low-enough capacity, deallocate that later and add it's capacity to the
                    // remainder group:
                    if (std::numeric_limits<skipfield_type>::max() - current_capacity >
                            remainder && /* <- to make sure we don't overflow in next line */
            ...
            if (remainder <
                min_block_capacity)  // Implies we were unable to consolidate remainder with an existing unused group, in the if-block above
            {
                // Note: negative_remainder is used to take the difference between the minimum block capacity limit and the actual
                // remainder, and spread this negative difference over subsequent blocks which are in the usual case at max capacity.
                negative_remainder = min_block_capacity - remainder;
                remainder          = min_block_capacity;

                // This line checks to see - if we have to reduce the size of the max-capacity blocks to spread the negative_remainder out -
                // whether even reducing the max blocks to min capacity will be enough to keep the capacity under max_size(). We add 1 for
                // the initial (remainder) block. This guards against situations where, for example, the min/max limits are very similar so
                ...
                if (max_size() - total_capacity < ((number_of_max_groups + 1) * min_block_capacity)) {
                    ...
                    throw std::length_error(
                        "Reserve cannot increase capacity to >= n without being > max_size() due to current capacity() and block capacity "
                        "limits");
#else
                    std::terminate();
#endif
        ...
        } else  // Non-empty hive, add first new unused group:
        {
            ...
            // We've now successfully allocated another group which is guaranteed to be larger than this group, so capacity is larger than
            // it was before reserve() was called even if the other allocations below trigger an exception, and we can deallocate the group:
   ...
   private:
    ...
    colony_iterator<is_const> get_it(const pointer element_pointer) const PLF_NOEXCEPT {
        if (end_iterator.group_pointer != nullptr) {
            ...
            // Note: we start with checking the back group first, as it will be the largest group in most cases, so there's a
            // statistically-higher chance of the element being within it.

            // Special case for back group in case the element was in a group which became empty and got moved to the unused_groups list or
            // was deallocated, and then that memory was re-used (ie. it became the current back group). The following prevents the function
            // from mistakenly returning an iterator which is beyond the back element of the colony:
            if (aligned_element_pointer >= end_iterator.group_pointer->front() && aligned_element_pointer < end_iterator.element_pointer) {
                const skipfield_pointer_type skipfield_pointer =
                    end_iterator.group_pointer->skipfield + (aligned_element_pointer - end_iterator.group_pointer->front());
                // The first test below checks to see whether the element is live or erased. The second test checks to see if the pointer
                // points to an erased element in a block of memory which got deallocated by the container, then another block got allocated
                // by the container which contained that memory space, but the new block is not exactly aligned with the old block. eg.
                // colony<structure> where structure is struct {int x, y} and the new memory block is offset by 1 int such that
                // 'element_pointer' points at y.
                return (*skipfield_pointer == 0 &&
                        ((reinterpret_cast<char*>(element_pointer) - reinterpret_cast<char*>(end_iterator.group_pointer->elements)) %
                             sizeof(aligned_element_struct) ==
                         0))
                           ? colony_iterator<is_const>(end_iterator.group_pointer, aligned_element_pointer, skipfield_pointer)
                           : colony_iterator<is_const>(end_iterator);
            }
            ...
            for (group_pointer_type current_group = end_iterator.group_pointer->previous_group; current_group != nullptr;
                ...
                if (aligned_element_pointer >= current_group->front() && aligned_element_pointer < current_group->past_back()) {
                    const skipfield_pointer_type skipfield_pointer =
                        current_group->skipfield + (aligned_element_pointer - current_group->front());
                    return (*skipfield_pointer == 0 &&
                            ((reinterpret_cast<char*>(element_pointer) - reinterpret_cast<char*>(current_group->elements)) %
                                 sizeof(aligned_element_struct) ==
                             0))
                               ? colony_iterator<is_const>(current_group, aligned_element_pointer, skipfield_pointer)
                               : colony_iterator<is_const>(end_iterator);
                }
            }
        }
   ...
   public:
    iterator get_iterator(const pointer element_pointer) PLF_NOEXCEPT { return get_it<false>(element_pointer); }
    ...
    bool is_active(const const_iterator& it) const PLF_NOEXCEPT {
        if (end_iterator.group_pointer != nullptr) {
            // Schema: check (a) that the group the iterator belongs to is still active and not deallocated or in the unused_groups list,
            // then (b) that the element is not erased. (a) prevents an out-of-bounds memory access if the group is deallocated. Same
            // reasoning as get_iterator for loop conditions

            // Special case for back group, same reasoning as in get_it():
            if (it.group_pointer == end_iterator.group_pointer && it.element_pointer >= end_iterator.group_pointer->front() &&
                it.element_pointer < end_iterator.element_pointer) {
                return (*it.skipfield_pointer == 0);
            }

            for (group_pointer_type current_group = end_iterator.group_pointer->previous_group; current_group != nullptr;
                 current_group                    = current_group->previous_group) {
                if (it.group_pointer == current_group && it.element_pointer >= current_group->front() &&
                    it.element_pointer <
                        current_group
                            ->past_back())  // 2nd 2 conditions necessary in case the group contained the element which the iterator points
                                            // to, has been deallocated from the colony previously, but then the same pointer address is
                                            // re-supplied via an allocator for a subsequent group allocation (in which case the group's
                                            // element block memory location may be different)
                {
                    return (*it.skipfield_pointer == 0);
                }
            }
    ...
    allocator_type get_allocator() const PLF_NOEXCEPT { return static_cast<allocator_type>(*this); }

   private:
    void source_blocks_incompatible() {
#ifdef PLF_EXCEPTIONS_SUPPORT
        throw std::length_error(
            "A source memory block capacity is outside of the destination's minimum or maximum memory block capacity limits - please "
            "change either the source or the destination's min/max block capacity limits using reshape() before calling splice() in this "
            "case");
#else
        std::terminate();
#endif
   ...
   public:
    void splice(colony& source) {
        ...
        // If the source has more unused memory spaces in the back group than the destination, swap them before processing to reduce the
        // number of locations added to a free list and also subsequent jumps during iteration.
        ...
        if (source.min_block_capacity > max_block_capacity ||
            source.max_block_capacity < min_block_capacity)  // ie. source blocks cannot possibly fit within *this's block capacity limits
        {
            source_blocks_incompatible();
        } else if (source.min_block_capacity < min_block_capacity ||
                   source.max_block_capacity > max_block_capacity)  // ie. source blocks may or may not fit
        {
        ...
        if (total_size != 0) {
            // If there's more unused element locations in back memory block of destination than in back memory block of source, swap with
            // source to reduce number of skipped elements during iteration:
            if ((end_iterator.group_pointer->past_back() - end_iterator.element_pointer) >
                (source.end_iterator.group_pointer->past_back() - source.end_iterator.element_pointer)) {
                swap(source);
            ...
            if (source.erasure_groups_head != nullptr) {
                if (erasure_groups_head != nullptr) {
                    ...
                    while (tail_group->erasures_list_next_group != nullptr) { tail_group = tail_group->erasures_list_next_group; }
            ...
            const skipfield_type distance_to_end =
                static_cast<skipfield_type>(end_iterator.group_pointer->past_back() - end_iterator.element_pointer);
            ...
            {                          // Mark unused element memory locations from back group as skipped/erased:
                ...
                if (previous_node_value == 0)  // no previous skipblock
                {
                    ...
                    *(end_iterator.skipfield_pointer + distance_to_end - 1) = distance_to_end;
                    ...
                    const skipfield_type index =
                        static_cast<skipfield_type>(end_iterator.element_pointer - end_iterator.group_pointer->front());

                    if (!end_iterator.group_pointer->no_erasures()) {
                        edit_free_list_next(end_iterator.group_pointer->front() + end_iterator.group_pointer->free_list_head,
                                            index);  // set prev free list head's 'next index' number to the index of the current element
                    } else {
                ...
                } else {  // update previous skipblock, no need to update free list:
                    *(end_iterator.skipfield_pointer - previous_node_value) = *(end_iterator.skipfield_pointer + distance_to_end - 1) =
                        static_cast<skipfield_type>(previous_node_value + distance_to_end);
        ...
        if (original_unused_groups_head != nullptr)  // If there were unused groups in source, re-link them and remove their capacity count
                                                     // from *this while adding it to source:
        {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    void splice(colony&& source) { splice(source); }
#endif
   ...
   private:
    struct item_index_tuple {
        ...
        item_index_tuple(const pointer _item, const size_type _index) PLF_NOEXCEPT : original_location(_item), original_index(_index) {}
    };
    ...
    struct sort_dereferencer {
        ...
        explicit sort_dereferencer(const comparison_function& function_instance) : stored_instance(function_instance) {}
    ...
    aligned_pointer_type get_free_space() const PLF_NOEXCEPT {
        const size_type number_of_elements_needed =
            ((total_size * sizeof(the_type)) + sizeof(aligned_element_struct) - 1) / sizeof(aligned_element_struct);  // rounding up

        if (number_of_elements_needed < max_block_capacity) {
            if (static_cast<size_type>(end_iterator.group_pointer->past_back() - end_iterator.element_pointer) >=
                number_of_elements_needed) {  // there is enough space at the back of the back block
            ...
            for (group_pointer_type current = unused_groups_head; current != nullptr; current = current->next_group) {
                if (current->capacity >= number_of_elements_needed)
                    return current->front();  // ie. there is enough space in one of the unused blocks
            }
        }
   ...
   public:
    ...
    void sort(comparison_function compare) {
        ...
        if PLF_CONSTEXPR ((std::is_trivially_copyable<element_type>::value || std::is_move_assignable<element_type>::value) &&
                          sizeof(element_type) <= sizeof(element_type*) * 2)  // If element is <= 2 pointers, just copy to an array and sort
                                                                              // that then copy back - consumes less memory
#else
...
#endif
        {
            ...
            if (need_to_allocate) { sort_array = PLF_ALLOCATE(allocator_type, *this, total_size, end_iterator.skipfield_pointer); }

            const pointer end = sort_array + total_size;

#if defined(PLF_TYPE_TRAITS_SUPPORT) && defined(PLF_MOVE_SEMANTICS_SUPPORT)
            if PLF_CONSTEXPR (!std::is_trivially_copy_constructible<element_type>::value &&
...
#if defined(PLF_TYPE_TRAITS_SUPPORT) && defined(PLF_MOVE_SEMANTICS_SUPPORT)
            if PLF_CONSTEXPR (!std::is_trivially_copy_assignable<element_type>::value &&
...
#endif
                {
                    for (element_type* current = sort_array; current != end; ++current) { PLF_DESTROY(allocator_type, *this, current); }
                }
            }

            if (need_to_allocate) { PLF_DEALLOCATE(allocator_type, *this, sort_array, total_size); }
        } else {
            ...
            if (need_to_allocate) { PLF_DEALLOCATE(tuple_allocator_type, tuple_allocator, sort_array, total_size); }
        }
    }

    void sort() { sort(plf::less<element_type>()); }
    ...
    size_type unique(comparison_function compare) {
        ...
        for (const_iterator current = begin_iterator, previous = begin_iterator; ++current != end; previous = current) {
            if (compare(*current, *previous)) {
                ...
                while (++last != end && compare(*last, *previous)) { ++count; }
    ...
    size_type unique() { return unique(std::equal_to<element_type>()); }
...
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
        if PLF_CONSTEXPR (std::allocator_traits<allocator_type>::is_always_equal::value &&
                          std::is_trivially_copyable<group_pointer_type>::value)  // if all pointer types are trivial we can just copy using
                                                                                  // memcpy - avoids constructors/destructors etc and is
                                                                                  // faster
        {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        else if PLF_CONSTEXPR (std::is_move_assignable<group_pointer_type>::value &&
...
#endif
        {
            // Otherwise, make the reads/writes as contiguous in memory as-possible (yes, it is faster than using std::swap with the
...
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
            if PLF_CONSTEXPR (std::allocator_traits<allocator_type>::propagate_on_container_swap::value &&
                              !std::allocator_traits<allocator_type>::is_always_equal::value)
#endif
            {
    ...
    // Because it's going to take a lot of defining of very specific allocator and pointer types to make this work with smart pointers -
    // this class and functions associated with it will not work if the allocator supplied to the container returns smart pointers.
    struct colony_data : public uchar_allocator_type {
        ...
        unsigned char** const bitfield_pointers;  // array of pointers to bitfields in the form of unsigned char arrays representing whether
                                                  // an element is erased or not (0 for erased).
        std::size_t* const block_capacities;      // array of the number of elements in each memory block
        const std::size_t  number_of_blocks;      // size of each of the arrays above

        colony_data(const typename colony::size_type size)
            : block_pointers(pointer_cast<aligned_pointer_type*>(
                  PLF_ALLOCATE(uchar_allocator_type, *this, size * sizeof(aligned_pointer_type), nullptr))),
              bitfield_pointers(
                  pointer_cast<unsigned char**>(PLF_ALLOCATE(uchar_allocator_type, *this, size * sizeof(unsigned char*), nullptr))),
              block_capacities(pointer_cast<std::size_t*>(PLF_ALLOCATE(uchar_allocator_type, *this, size * sizeof(std::size_t), nullptr))),
              number_of_blocks(size) {}

        ~colony_data() {
            for (std::size_t index = 0; index != number_of_blocks; ++index) {
                PLF_DEALLOCATE(uchar_allocator_type, *this, bitfield_pointers[index], (block_capacities[index] + 7) / 8);
            }

            PLF_DEALLOCATE(uchar_allocator_type, *this, pointer_cast<unsigned char*>(block_pointers),
                           number_of_blocks * sizeof(aligned_pointer_type));
            PLF_DEALLOCATE(uchar_allocator_type, *this, pointer_cast<unsigned char*>(bitfield_pointers),
                           number_of_blocks * sizeof(unsigned char*));
            PLF_DEALLOCATE(uchar_allocator_type, *this, pointer_cast<unsigned char*>(block_capacities),
                           number_of_blocks * sizeof(std::size_t));
        }
    };

   private:
    void setup_data_cell(colony_data* data, const group_pointer_type current_group, const std::size_t capacity,
                         const std::size_t group_number) {
        const std::size_t bitfield_capacity = (capacity + 7) / 8;  // round up

        data->block_pointers[group_number] = current_group->front();
        unsigned char* bitfield_location   = data->bitfield_pointers[group_number] =
            PLF_ALLOCATE(uchar_allocator_type, (*data), bitfield_capacity, nullptr);
        ...
        for (std::size_t index = 0; bitfield_location != end; ++bitfield_location) {
   ...
   public:
    colony_data* data() {
        ...
        for (group_pointer_type current_group = begin_iterator.group_pointer; current_group != end_iterator.group_pointer;
             current_group                    = current_group->next_group, ++group_number) {
            setup_data_cell(data, current_group, current_group->capacity, group_number);
        }

        // Special case for end group:
        setup_data_cell(data, end_iterator.group_pointer,
                        static_cast<std::size_t>(end_iterator.element_pointer - end_iterator.group_pointer->front()), group_number);
    ...
    class colony_iterator {
       ...
       public:
        ...
        bool operator==(const colony_iterator& rh) const PLF_NOEXCEPT { return (element_pointer == rh.element_pointer); }

        bool operator==(const colony_iterator<!is_const>& rh) const PLF_NOEXCEPT { return (element_pointer == rh.element_pointer); }
        ...
        pointer operator->() const { return pointer_cast<pointer>(element_pointer); }

        colony_iterator& operator++() {
            plf_assert(group_pointer != nullptr);  // covers uninitialised colony_iterator

#if defined(_MSC_VER) && !defined(__clang__) && \
    !defined(                                   \
        __GNUC__)  // This version creates significantly faster release code under MSVC in some scenarios, but not in any other compilers
            element_pointer += *(++skipfield_pointer) + 1;
            skipfield_pointer += *skipfield_pointer;

            if (element_pointer == group_pointer->past_back() &&
                group_pointer->next_group != nullptr)  // Second condition allows iterator to reach end(), which may be 1 past end of block,
                                                       // if block has been fully used
            {
            ...
            if ((element_pointer += static_cast<size_type>(skip) + 1u) == group_pointer->past_back() &&
        ...
        colony_iterator& operator--() {
            plf_assert(group_pointer != nullptr);
            ...
            if (--skipfield_pointer >= group_pointer->skipfield)  // ie. not already at beginning of group prior to decrementation
            {
                ...
                if ((skipfield_pointer -= *skipfield_pointer) >= group_pointer->skipfield)
                    return *this;  // ie. skipfield jump value does not takes us beyond beginning of group
            }
        ...
        std::strong_ordering operator<=>(const colony_iterator<is_const_it>& rh) const noexcept {
            return (element_pointer == rh.element_pointer) ? std::strong_ordering::equal
       ...
       private:
        ...
        colony_iterator(const group_pointer_type group_p, const aligned_pointer_type element_p,
                        const skipfield_pointer_type skipfield_p) PLF_NOEXCEPT : group_pointer(group_p),
        ...
        aligned_pointer_type get_end() const PLF_NOEXCEPT {
            ...
            if (group_pointer->next_group == nullptr) {
                colony_iterator current(group_pointer, nullptr, 0);
        ...
        {
            plf_assert(group_pointer != nullptr);  // covers uninitialized colony_iterator && empty group
            ...
            if (distance > 0)  // ie. +=
            {
                ...
                // 2. If any of the first group's elements have been erased, we manually iterate, as we don't know whether the erased
                // elements occur before or after the initial iterator position, and we subtract 1 from the distance amount each time we
                // iterate. Iteration continues until either distance becomes zero, or we reach the end of the group.

                // For all subsequent groups, we follow this logic:
                // 1. If distance is larger than the total number of non-erased elements in a group, we skip that group and subtract the
                ...
                //   a. If there are no erased elements in the group we simply add distance to group->elements to find the new location for
                //   the iterator. b. If there are erased elements in the group, we manually iterate and subtract 1 from distance on each
                //   iteration, until the new iterator location is found ie. distance = 0.
                ...
                if (element_pointer !=
                    group_pointer
                        ->first_element())  // Otherwise we skip this section and treat the first block as we would an intermediary block
                {
                    ...
                    if (group_pointer->no_erasures()) {
                        if (distance < distance_from_end) {
                            ...
                            return;
                                 ...
                                 nullptr)  // either we're going to reach end() or go beyond it, so bound to back of block
                        {
                            element_pointer = end;
                ...
                // Intermediary groups - at the start of this code block and the subsequent block, the position of the iterator is assumed
                // to be the first non-erased element in the current group:
                while (static_cast<difference_type>(group_pointer->size) <= distance) {
                    if (group_pointer->next_group ==
                        nullptr)  // either we would reach end() or go beyond it, so bound to end() - necessary to reach rbegin()
                    {
                ...
                // We already know group size > distance due to the intermediary group checks above - so it's safe to ignore endpoint check
                // condition while incrementing here.
                advance_from_group_beginning(static_cast<skipfield_type>(distance));
            } else if (distance < 0) {
                if (group_pointer->previous_group == nullptr && element_pointer == group_pointer->first_element())
                    return;  // if we are at begin(), bound to that

                distance = -distance;

                // Special case for initial element pointer and initial group (we don't know how far into the group the element pointer is)
                if (element_pointer !=
                    group_pointer->past_back())  // Optimization: if end() is cheaply calculable (edge case, is one past end of block), and
                                                 // iterator == end(), skip this first section and treat current group like an intermediary
                                                 // group. Note: only end() can be 1-past end of block, no other iterator.
                {
                    if (group_pointer->no_erasures()) {
                        const difference_type distance_from_beginning =
                            static_cast<difference_type>(element_pointer - group_pointer->front());

                        if (distance <= distance_from_beginning) {
                            element_pointer -= distance;
                            skipfield_pointer -= distance;
                            return;
                        } else if (group_pointer->previous_group == nullptr)  // we've gone before begin(), so bound to that
                        {
                            element_pointer   = group_pointer->front();
                            skipfield_pointer = group_pointer->skipfield;
                            return;
                        ...
                        for (const skipfield_pointer_type begin = group_pointer->skipfield + *(group_pointer->skipfield);
                             skipfield_pointer != begin;) {
                            --skipfield_pointer;
                            skipfield_pointer -= *skipfield_pointer;
                ...
                // Intermediary groups - at the start of this code block and the subsequent block, the position of the iterator is assumed
                // to be either the first non-erased element in the next group over, or end():
                while (static_cast<difference_type>(group_pointer->size) < distance) {
        ...
        difference_type distance(const colony_iterator& last) const {
            ...
            // In the initial and final groups, manual incrementation must be used to calculate distance, if there have been any erasures in
            // those groups. If there are no prior erasures in either of those groups we can use pointer arithmetic to calculate the
            // distances.

            plf_assert(!(group_pointer == nullptr) && !(last.group_pointer == nullptr));  // Check that they are both initialized
            ...
            if (iterator1.group_pointer != iterator2.group_pointer)  // if not in same group, process intermediate groups
            {
                ...
                {
                    for (const skipfield_pointer_type end =
                             iterator1.skipfield_pointer + (iterator1.group_pointer->past_back() - iterator1.element_pointer);
                         iterator1.skipfield_pointer != end;) {
            ...
            } else if (iterator1.element_pointer == iterator2.group_pointer->first_element() &&
                       iterator2.element_pointer + 1 + *(iterator2.skipfield_pointer + 1) ==
                           iterator2.group_pointer
                               ->past_back())  // ie. if iterator1 is at beginning of block (have to check this in case first and last are
                                               // in the same block to begin with) and iterator2 is last element in the block. This check
                                               // won't work for back block (as end() may not be past end of block), but that's fine, it's
    ...
    // Reverse iterators:
    // Note: despite iterator being trivially_copyable, defining reverse_iterator via std::reverse_iterator<iterator> results in it not
    // being trivially_copyable here, at least under libstdc++. Hence we have a custom implementation.
    ...
    class colony_reverse_iterator {
       ...
       public:
        ...
        friend typename colony_reverse_iterator::difference_type distance(const colony_reverse_iterator& first,
                                                                          const colony_reverse_iterator& last) {
            return first.distance(last);
        }
        ...
        colony_reverse_iterator(const colony_iterator<is_const_r>& source) PLF_NOEXCEPT : current(source) {}
        ...
        bool operator==(const colony_reverse_iterator& rh) const PLF_NOEXCEPT { return (current == rh.current); }

        bool operator==(const colony_reverse_iterator<!is_const_r>& rh) const PLF_NOEXCEPT { return (current == rh.current); }

        bool operator!=(const colony_reverse_iterator& rh) const PLF_NOEXCEPT { return (current != rh.current); }

        bool operator!=(const colony_reverse_iterator<!is_const_r>& rh) const PLF_NOEXCEPT { return (current != rh.current); }

        reference operator*() const PLF_NOEXCEPT { return *--iterator(current); }

        pointer operator->() const PLF_NOEXCEPT { return pointer_cast<pointer>(--iterator(current).element_pointer); }
        ...
        colony_iterator<is_const_r> base() const PLF_NOEXCEPT { return colony_iterator<is_const_r>(current); }
       ...
       private:
        void advance(const difference_type distance) { current.advance(-distance); }

        difference_type distance(const colony_reverse_iterator& last) const { return last.current.distance(current); }
...
typename plf::colony<element_type, allocator_type>::size_type erase_if(plf::colony<element_type, allocator_type>& container,
    ...
    for (const_iterator current = container.cbegin(); current != end; ++current) {
        if (predicate(*current)) {
            ...
            while (++last != end && predicate(*last)) { ++count; }
...
template <class element_type, class allocator_type>
typename plf::colony<element_type, allocator_type>::size_type erase(plf::colony<element_type, allocator_type>& container,
